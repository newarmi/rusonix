<template>
    <section class="category">
        <div class="container">
            <h3 class="category__title">Результаты поиска по запросу "{{search}}"</h3>
            <ul class="category__list">
                <li v-for="result in description" :key="result.slug" class="category__item">
                    <div class="category__item-wrap">
                        <span class="category__item-img"><img src="@/assets/img/contact-icon.png" alt=""></span>
                        <div class="category__link" @click="openPage(result.slug, result.type)">{{result.name}}</div>
                    </div>
...
export default {
  ...
  methods: {
    openPage(post, type) {
      if(type==='main') this.$router.push({path: '/' + post})
      if(type==='journal') this.$router.push({path: '/journal/' + post})
      if(type==='service') this.$router.push({path: '/service/' + post})
    },
  }
}
</script>
