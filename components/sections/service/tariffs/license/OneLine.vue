  ...
  <section :id="lines.tag" class="license__storage">
    <div class="container">
      ...
      <div class="license__wrapper" :class="wrapper">
        <AntivirusDesktop v-if="lines.line[0].layout==='antivirus'" :lines="lines.line[0].attributes.license"
                          :button="button"/>
      </div>

      <!-- tablet -->
      <div class="antispam__tablet">
        <AntivirusTablet v-if="lines.line[0].layout==='antivirus'" :lines="lines.line[0].attributes.license"
                         :button="button"/>
      </div>
    </div>
  </section>
...
export default {
  name: 'OneLine',
  components: {
    AntivirusDesktop: () => import('~/components/sections/service/tariffs/license/AntivirusDesktop'),
    AntivirusTablet: () => import('~/components/sections/service/tariffs/license/AntivirusTablet'),
  },
  props: {
    lines: {
      type: Object,
  ...
  computed: {
    button() {
      return {title: this.lines.buttonName,
              modal: this.lines.modal,
              link: this.lines.buttonLink}
    },
    wrapper() {
      return {
        'license__wrapper-two': this.optionNumber === 1,
